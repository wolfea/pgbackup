...
class DriverAction(Action):
	...
	def __call__(self, parser, namespace, values, option_string=None):
		...
		#check to see if driver (in lower case) is in the known_drivers list (if not, raise error)  
...
def create_parser():
	parser=ArgumentParser("Backup PostgreSQL database locally or to AWS S3")
	parser.add_argument("url", help="URL of the database to backup")
	parser.add_argument("--driver", help="how and where to store the backup", nargs=2, required=True, 
				action=DriverAction)    
